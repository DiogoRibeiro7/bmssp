...
class BMSSPProfiled:
    ...
    def _bmssp(self, graph: nx.DiGraph, level: int, bound: float, 
              sources: set, k: int, t: int) -> Tuple[float, set]:
        self.profiler.operations += 1
    ...
    def _find_pivots(self, graph: nx.DiGraph, bound: float, 
                    sources: set, k: int) -> Tuple[set, set]:
    ...
    def _base_case(self, graph: nx.DiGraph, bound: float, 
                  sources: set, k: int) -> Tuple[float, set]:
...
class PerformanceAnalyzer:
    ...
    def run_comparison(self, graph_sizes: List[int], 
        ...
        for graph_type in graph_types:
            ...
            for n in graph_sizes:
                ...
                self.results[graph_type].append({
                    ...
                    'bmssp': avg_bmssp,
                    'runtime_improvement': (avg_dijkstra.runtime_ms - avg_bmssp.runtime_ms) / avg_dijkstra.runtime_ms,
                    'memory_improvement': (avg_dijkstra.memory_mb - avg_bmssp.memory_mb) / max(avg_dijkstra.memory_mb, 1),
                    'theoretical_improvement': (avg_dijkstra.theoretical_complexity - avg_bmssp.theoretical_complexity) / avg_dijkstra.theoretical_complexity
                })
                
                print(f"✓ (Runtime: {avg_bmssp.runtime_ms:.1f}ms vs {avg_dijkstra.runtime_ms:.1f}ms)")
    ...
    def _average_metrics(self, metrics_list: List[PerformanceMetrics]) -> PerformanceMetrics:
        ...
        return PerformanceMetrics(
            ...
            vertices_processed=int(np.mean([m.vertices_processed for m in metrics_list])),
    ...
    def generate_report(self, results: Dict) -> str:
        ...
        for graph_type, data in results.items():
            ...
            if crossover_n:
                report.append(f"**Crossover Point**: BMSSP becomes faster at n ≥ {crossover_n}")
            else:
                report.append("**No crossover observed** in tested range")
            report.append("")
            
            # Performance table
            report.append("| n | m | Dijkstra (ms) | BMSSP (ms) | Runtime Improvement | Memory Improvement |")
            report.append("|---|---|---------------|------------|--------------------|--------------------|")
            ...
            for entry in data:
                ...
                report.append(f"| {n} | {m} | {d_time:.1f} | {b_time:.1f} | {runtime_imp:+.1f}% | {memory_imp:+.1f}% |")
            ...
            # Memory analysis
            avg_memory_improvement = np.mean([entry['memory_improvement'] for entry in data]) * 100
            report.append(f"**Average memory improvement**: {avg_memory_improvement:.1f}%")
            report.append("")
        
        # Theoretical vs empirical
        report.append("## Theoretical vs Empirical Complexity")
        report.append("")
        report.append("The theoretical advantage of BMSSP (O(m log^(2/3) n) vs O(m + n log n))")
        ...
        for graph_type, data in results.items():
            if len(data) > 1:
                ...
                report.append(f"- **{graph_type}** (n={large_graph['n']}): "
    ...
    def plot_results(self, results: Dict, save_path: str = "performance_analysis.png"):
        ...
        for i, (graph_type, data) in enumerate(results.items()):
            ...
            ax1.loglog(n_values, dijkstra_times, f'{color}--', label=f'Dijkstra ({graph_type})')
            ax1.loglog(n_values, bmssp_times, f'{color}-', label=f'BMSSP ({graph_type})')
        ...
        for i, (graph_type, data) in enumerate(results.items()):
            ...
            ax4.semilogx(n_values, theoretical, f'{color}--', alpha=0.7, label=f'Theory ({graph_type})')
            ax4.semilogx(n_values, empirical, f'{color}-', label=f'Empirical ({graph_type})')
...
def main():
    ...
    for graph_type, data in results.items():
        if data:
            ...
            crossover_point = next((entry['n'] for entry in data if entry['runtime_improvement'] > 0), None)
            
            if crossover_point:
                print(f"   {graph_type}: BMSSP faster at n≥{crossover_point}, max improvement {best_improvement:.1f}%")
            else:
                print(f"   {graph_type}: No crossover in tested range (overhead dominates)")
